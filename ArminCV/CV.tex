...
$\diamond$
\textbf{DRAM Memory} -
DRAM memories need refresh operations because they lose their content/charge over time. The overhead of these refreshes increases with
...
My B.Sc. thesis project is on predicting the correlation of reuse-distance between cache blocks and remaining hit count of each cache block. In our research, we proposed a cache replacement policy that leverages the correlation between reuse-distance and the remaining hit count.
}
...
\SubBulletItem {Introduction to Computer Architecture (CMPEN 431) \hfill{Spring 2020}}
