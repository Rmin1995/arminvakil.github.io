...
Applications with big memory footprint experience high number of page walks during their execution, that leads to high 
performance degradation, especially in virtualized systems. In this project, we first characterize the page walk memory
accesses to identify the main overhead of page walk, and then propose different type of optimizations throughout the 
memory hierarchy to reduce this overhead.
...
$\diamond$
\textbf{DRAM} -
DRAM needs refresh operations because DRAM cells lose their content/charge over time. The overhead of these refreshes increases with
larger DRAM memories. My research in this area reduces the memory refresh overhead in virtualized systems by leveraging the same-content values in DRAM.
\newline
$\diamond$
\textbf{Persistent Memory} -
Memory persistency models constrain the order of reaching persistent writes to persistent memory (PM). To enforce this order in x86 processors, programmers should use expensive \texttt{sfence} instructions. In this project, we proposed an extension to x86 memory persistency model based on two existing paths to PM, which enables providing implicit persist ordering without using \texttt{sfence} instructions.
...
\SubBulletItem {\href{https://www.youtube.com/watch?v=85qSqmwk0ss}{ACM SIGMETRICS} \hfill{June 2020}}
